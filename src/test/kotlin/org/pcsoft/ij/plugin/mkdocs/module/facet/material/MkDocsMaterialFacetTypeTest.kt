...
class MkDocsMaterialFacetTypeTest : BasePlatformTestCase() {
    ...
    fun `test is badged onto the mkdocs icon without growing it`() {
        ...
        assertEquals("badging must not widen the icon", MkDocsIcons.MkDocs.iconWidth, icon!!.iconWidth)
        assertEquals("badging must not heighten the icon", MkDocsIcons.MkDocs.iconHeight, icon.iconHeight)
    }
