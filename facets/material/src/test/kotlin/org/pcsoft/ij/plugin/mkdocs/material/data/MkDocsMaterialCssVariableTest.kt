...
class MkDocsMaterialCssVariableTest {
    ...
    private val cssVariables = MkDocsMaterialDataService().cssVariables
    ...
    fun `the bundled resource parses`() {
        assertTrue(cssVariables.all.isNotEmpty())
    }
    ...
    fun `every variable carries the theme prefix`() {
        cssVariables.all.forEach { variable ->
    ...
    fun `names are unique`() {
        val names = cssVariables.all.map { it.name }
    ...
    fun `only colour variables are marked as colours`() {
        assertTrue(cssVariables.byName("--md-primary-fg-color")?.isColor == true)
        assertTrue(cssVariables.byName("--md-shadow-z1")?.isColor == false)
        assertTrue(cssVariables.byName("--md-text-font")?.isColor == false)
    }
    ...
    fun `every group holds at least one variable`() {
        MkDocsMaterialCssVariableGroup.entries.forEach { group ->
            assertTrue(group.name, cssVariables.byGroup(group).isNotEmpty())
        }
    }
    ...
    fun `byName rejects unknown properties`() {
        assertNull(cssVariables.byName("--md-nonsense-color"))
        assertNull(cssVariables.byName("--my-own-color"))
    }
}
