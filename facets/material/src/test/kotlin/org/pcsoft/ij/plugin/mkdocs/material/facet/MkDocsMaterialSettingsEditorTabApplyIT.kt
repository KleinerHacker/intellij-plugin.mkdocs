...
class MkDocsMaterialSettingsEditorTabApplyIT : HeavyPlatformTestCase() {
    ...
    /** The module holding the site, created by [createSite]. */
    private lateinit var siteModule: Module
    ...
    private fun pages(): Pages {
        val context = TestFacetEditorContext(siteModule)
    ...
    /**
     * Writes an `mkdocs.yml` with [text] and puts a module around it, so a tab built on that module finds the
     * configuration file the way it does in the IDE.
     *
     * The module is created here rather than left to the detection of the plugin: what this test is about is
     * the tabs, and a feature must not reach into the plugin to set its own tests up. The tabs ask
     * `MkDocsSiteFiles` for the configuration file of a module, so a module with the site in its content root
     * is exactly the state they need.
     *
    ...
    private fun createSite(text: String): VirtualFile {
        ...
        siteModule = createModule("Handbook")
        PsiTestUtil.addContentRoot(siteModule, configFile.parent)
        return configFile
    }
