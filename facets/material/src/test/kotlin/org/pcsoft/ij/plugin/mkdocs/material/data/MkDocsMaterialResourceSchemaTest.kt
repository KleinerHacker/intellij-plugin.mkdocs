 ...
 *
 * Every YAML resource under `material/spec` names a JSON schema next to it, which is what validates the
...
class MkDocsMaterialResourceSchemaTest {
    ...
    /**
     * Use case: `css-variables.yaml` fills the completion of an additional style sheet.
     */
    @Test
    fun `the css variables resource matches its schema`() {
    ...
    fun `every resource points at its schema`() {
        listOf("markdown-extensions", "feature-flags", "extra-keys", "fonts", "colors", "css-variables")
            .forEach { name ->
                val text = resource("/material/spec/$name.yaml").reader(Charsets.UTF_8).use { it.readText() }
                assertTrue(
    ...
    private fun validate(name: String) {
        val document = resource("/material/spec/$name.yaml").use { yaml.readTree(it) }
        val schema = resource("/material/spec/$name.schema.json").reader(Charsets.UTF_8)
