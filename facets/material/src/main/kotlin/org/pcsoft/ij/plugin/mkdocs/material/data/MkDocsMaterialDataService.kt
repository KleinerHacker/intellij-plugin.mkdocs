 ...
 * YAML resources under `material/spec`, each one described by a JSON schema next to it, so the data can be
...
class MkDocsMaterialDataService {
    ...
    /** The `--md-*` custom properties the theme styles itself with. */
    val cssVariables: MkDocsMaterialCssVariables by lazy {
        MkDocsMaterialCssVariables(readList<CssVariableEntry>(RESOURCE_CSS_VARIABLES).map {
            MkDocsMaterialCssVariable(it.name, it.group, it.isColor, it.descriptionKey)
        })
    }
    ...
    companion object {
        ...
        const val RESOURCE_EXTENSIONS: String = "/material/spec/markdown-extensions.yaml"

        /** The classpath resource describing the feature flags. */
        const val RESOURCE_FEATURE_FLAGS: String = "/material/spec/feature-flags.yaml"

        /** The classpath resource describing the keys below `extra`. */
        const val RESOURCE_EXTRA_KEYS: String = "/material/spec/extra-keys.yaml"

        /** The classpath resource describing the curated fonts. */
        const val RESOURCE_FONTS: String = "/material/spec/fonts.yaml"

        /** The classpath resource describing the palette colours. */
        const val RESOURCE_COLORS: String = "/material/spec/colors.yaml"

        /** The classpath resource describing the CSS custom properties. */
        const val RESOURCE_CSS_VARIABLES: String = "/material/spec/css-variables.yaml"
    }
}
...
private class CssVariableEntry(
    val name: String,
