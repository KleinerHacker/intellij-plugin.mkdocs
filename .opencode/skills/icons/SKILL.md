---
name: icons
description: Icon rules for the IntelliJ UI of this plugin - drawing an SVG on the 48x48 canvas, dark/light variants, where the file lives per module, registration through the *Icons object and the render size per place. Read before an icon is drawn, moved, renamed, registered or resized.
---

* MUST use the `icon-creator` agent to draw an icon
...
* An icon file lies in the `icons` folder of the resources of the module shipping it
  ...
  * A facet: below the `<facet-name>/` prefix its resources carry anyway, e.g. `material/icons` - the
    prefix is mandatory and ruled by the `facets` skill
...
* An icon is reached through the `*Icons` object of its module, NEVER loaded at the place rendering it
  ...
  * Where that object lies is ruled by `.claude/rules/architecture.md`: the package root of its module
