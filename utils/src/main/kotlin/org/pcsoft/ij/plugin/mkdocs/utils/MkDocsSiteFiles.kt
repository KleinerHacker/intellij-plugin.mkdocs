...
object MkDocsSiteFiles {
    ...
    /**
     * Locates the MkDocs configuration file of [module] without knowing what the site configured.
     *
     * What a facet of a feature asks: it is handed a module and needs the file the site is written in, but it
     * cannot read the configured path — that lives in the facet of the plugin, which a feature must not reach
     * into. The spelling MkDocs itself prefers is used instead, so a directory holding both `mkdocs.yml` and
     * `mkdocs.yaml` yields the same file the plugin picked.
     *
     * A `null` result doubles as the answer to "is this an MkDocs module": a module without a configuration
     * file holds no site.
     *
     ...
     */
    @JvmStatic
    fun findConfigFile(module: Module): VirtualFile? =
        findConfigFile(module, MkDocsProject.CONFIG_FILE_NAMES.first())
